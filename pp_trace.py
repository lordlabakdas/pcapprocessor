...
def pp_trace(pcap_file , unit, config, scenario, ascii_trace_file, buf_size):
    ...
    connections_list = [ pcap_trace_lines[ matches[j]:matches[j+1] ]  for j in range(len(matches)-1) ]
    ...
    for i in range (len(connections_list)) :
        ...
        try  :
            ...
            flow_cmp_time = ( t.time().hour * 60 * 60 +
                              t.time().minute * 60  +
                              t.time().second ) * 1000 + \
        ...
        overhead = PACKET_OVERHEAD * int( values[labels.index('total_packets_' + conn_suffix)] )
        ...
        throughput = int(values[labels.index('actual_data_bytes_' + conn_suffix)]) / (tx_time)
        rtt = float(values[labels.index('RTT_avg_' + conn_suffix)]) / 2
        utilization = throughput*100.0/bn_speed 
        
        # output formatting...
        #print host_a + ' -> ' +host_b
        result_str = [ tx_packts,
                       overhead , 
